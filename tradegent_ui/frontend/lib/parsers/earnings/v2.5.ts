/**
 * Earnings analysis parser — schema version 2.5
 *
 * Flat structure with a `decision` block carrying the recommendation.
 * No `scoring` section (added in v2.6).
 *
 * Key YAML paths:
 ...
 *   setup, market_environment, comparable_companies, liquidity_analysis, scoring
...
export function earningsParserV25(response: AnalysisDetailResponse): AnalysisDetail {
  ...
  // confidence: prefer gate column, fall back to decision block or probability
  ...
  return {
    _meta: {
      ...
      version: response.schema_version || get(meta, 'version', '2.5'),
    ...
    // setup: use what is available from root/preparation
    setup: {
    ...
    // catalyst: derive from preparation
    catalyst: {
      type:           'earnings',
    ...
    // market_environment: not present in v2.5
    market_environment: {
      regime:            'neutral',
      sector:            '',
