/**
 * Stock analysis parser — schema version 2.7
 *
 * Key YAML paths (identical to v2.6 except where noted):
 ...
 *
 * v2.7 additions over v2.6:
 *   price_alerts[].tag        — visualization label (e.g. "20-day MA")
 *   price_alerts[].derivation — methodology object
 *   summary.key_levels.*_derivation objects
 *   time_validation section (not surfaced in AnalysisDetail type)
 */
...
export function stockParserV27(response: AnalysisDetailResponse): AnalysisDetail {
  ...
  return {
    _meta: {
      ...
      version: response.schema_version || get(meta, 'version', '2.7'),
    ...
    // v2.7: price_alerts carry a `tag` field for visualization
    alert_levels: {
      price_alerts:     transformPriceAlerts(alerts.price_alerts, true),
